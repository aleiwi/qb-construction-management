...
def _sector_progress(sector_id: str, finishing_items: List[dict]) -> float:
...
def _donut_svg(value: float) -> str:
    size = 176
    r = size / 2 - 12
    cx = cy = size / 2
    ...
    <div style="display:flex;justify-content:center;gap:18px;margin-top:8px;font-size:10px;font-weight:700;">
...
def _header(d: dict, title: str, page_num: int, total: int = 6) -> str:
    ...
    <div style="border-bottom:2px solid #f59e0b;padding-bottom:7px;margin-bottom:10px;
      ...
      <div style="display:flex;align-items:center;gap:12px;font-size:9px;color:#64748b;direction:rtl;">
        ...
        <span style="width:1px;height:12px;background:#cbd5e1;"></span>
        <span>الصفحة: <strong dir="ltr" style="font-family:monospace;color:#d97706;">0{page_num} / 0{total}</strong></span>
      </div>
    </div>"""
...
def _footer(d: dict, page_num: int, total: int = 6) -> str:
    ...
    <div style="border-top:1px solid #e2e8f0;padding-top:6px;margin-top:auto;
      ...
      <span dir="ltr" style="font-family:monospace;font-weight:700;color:#475569;">0{page_num} / 0{total}</span>
    </div>"""
...
def _page(d: dict, page_num: int, body: str, title: str) -> str:
    return f"""
    <div class="page">
      {_header(d, title, page_num)}
      {body}
      {_footer(d, page_num)}
    </div>"""
...
def _build_html(d: Dict[str, Any]) -> str:
    ...
    meta_rows = "".join(
        f"""
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:8px 10px;
          display:flex;justify-content:space-between;align-items:center;">
          <span style="color:#64748b;font-weight:600;font-size:10px;">{k}</span>
          <span style="color:#0f172a;font-weight:800;font-size:10.5px;">{v}</span>
        </div>"""
        for k, v in [
    ...
    main_photo_html = ""
    if photo_gallery and photo_gallery[0].get("src"):
        p = photo_gallery[0]
        main_photo_html = f"""
        <div style="margin-top:10px;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;
          background:#0f172a;position:relative;height:120px;">
          <img src="{p.get('src')}" alt="" style="width:100%;height:100%;object-fit:cover;"/>
          <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(to top, rgba(2,6,23,0.95), rgba(2,6,23,0.4) 60%, transparent);
            padding:8px 10px;color:#fff;">
            <div style="font-size:10px;font-weight:900;color:#fbbf24;">{_esc(p.get('caption') or p.get('title') or 'الصورة الرئيسية للموقع')}</div>
            <div style="font-size:8.5px;color:#cbd5e1;">تصوير وتوثيق الموقع الميداني لمشروع {_esc(d.get('projectName'))}</div>
          </div>
        </div>"""
    ...
    </div>""", 'ملخص الإنجاز ومؤشرات المشروع')
    ...
    indicator_cards = "".join(
        ...
        <div style="background:{bg};border:1px solid {border};border-radius:12px;padding:10px 12px;
          ...
          <div style="width:36px;height:36px;border-radius:50%;background:{bg_circle};border:1px solid {circle_border};
            display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;color:{color};font-family:monospace;">
            {pct:.0f}%
          </div>
        </div>"""
        for label, pct, sub, bg, border, txt, color, bg_circle, circle_border in [
            ("متوسط الإنجاز الكلي", overall, f"إجمالي {total_items} بنداً هندسياً", "#0f172a", "#334155", "#fbbf24", "#fbbf24", "rgba(245,158,11,0.15)", "rgba(245,158,11,0.4)"),
            ("أعمال العظم (الهيكل)", st_pct, f"{len(structure_items)} بنداً · {'منجزة بالكامل' if st_pct == 100 else 'قيد التنفيذ'}", "#ecfdf5", "#a7f3d0", "#065f46", "#059669", "#059669", "#10b981"),
            ("أعمال التشطيبات", fn_pct, f"{len(finishing_items)} بنداً · قيد التنفيذ الجاري", "#fffbeb", "#fde68a", "#92400e", "#d97706", "#d97706", "#f59e0b"),
        ]
    )
    ...
    for idx, s in enumerate(finishing_sectors, start=1):
        ...
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:6px 9px;
          ...
          <div style="background:{color if pct > 0 else '#f1f5f9'};color:{color if pct > 0 else '#64748b'};
    ...
    </div>""", 'الإنجاز حسب مرحلتي العمل والقطاعات')
    ...
    </div>""", 'تفاصيل أعمال العظم (الهيكل والأساسات)')
    ...
    </div>""", 'ميزانية المشروع وبيان الدفعات')

    # ── Page 6: Photos & signatures ─────────────────────────────────────────
    photos_html = ""
    if photo_gallery:
        photo_cells = ""
        for p in photo_gallery[:6]:
            photo_cells += f"""
            <div style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;background:#0f172a;position:relative;">
              <img src="{p.get('src')}" alt="" style="width:100%;height:100%;object-fit:cover;"/>
              <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(to top, rgba(2,6,23,0.9), transparent);
                padding:7px;color:#fff;font-size:9px;font-weight:800;">{_esc(p.get('caption') or p.get('title') or '')}</div>
            </div>"""
        photos_html = f"""
        <div class="card" style="flex:1;display:flex;flex-direction:column;">
          {_section_title(8, 'توثيق مصور لسير العمل الميداني')}
    ...
    sig_blocks = "".join(
        f"""
        <div style="flex:1;text-align:center;background:#fff;padding:12px;border:1px solid #e2e8f0;border-radius:10px;">
          <div style="height:52px;border-bottom:2px dashed #cbd5e1;margin-bottom:7px;display:flex;align-items:flex-end;justify-content:center;padding-bottom:6px;">
            <span style="font-size:8.5px;color:#94a3b8;font-family:monospace;">التوقيع والختم</span>
          </div>
          <div style="font-size:11px;font-weight:900;color:#0f172a;">{title}</div>
          <div style="font-size:9px;color:#64748b;margin-top:2px;">{sub}</div>
        </div>"""
        for title, sub in [
            ("الاستشاري / المهندس المشرف", "اعتماد نسبة الإنجاز التنفيذي"),
            ("المطور العقاري", _esc(d.get('companyName') or 'مسقا الأولى للتطوير العقاري')),
            ("المقاول الرئيسي للمشروع", "تأكيد الميزانية والبنود المنجزة"),
        ]
    )

    sig_section = f"""
    <div class="card" style="{'flex:1;display:flex;flex-direction:column;justify-content:center;' if not photos_html else 'margin-top:auto;'}">
      {_section_title(9 if photos_html else 8, 'توقيعات واعتمادات أطراف المشروع المعتمدة')}
    ...
    'التوثيق المصور للموقع والاعتمادات' if photos_html else 'توقيعات واعتمادات أطراف المشروع')
...
async def generate_completion_pdf(report_data: Dict[str, Any]) -> bytes:
    """
    Render the HTML report in a headless Chromium and export as PDF bytes.
    """
